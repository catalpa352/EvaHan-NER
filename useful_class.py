...
class dataprocess(Dataset):
    ...
    def map_tags_to_ids(self, tags2):
        ...
        # # 标签列表
        labels = [
            "O", "B-NS", "E-NS", "S-NS", "B-NR", "E-NR",
            "B-T", "E-T", "M-NR", "S-NR", "M-NS", "M-T", "S-T"
        ]
...
class dataprocess2(Dataset):
    ...
    def split_into_segments2_testdata(self, lines, max_length=28):
        """
        将CoNLL格式的行列表转换为训练数据格式，根据标点切分句子，同时确保实体完整性。
    ...
    def hybrid_split_testdata(self, lines, max_length=28, min_length=10, stop_words=None):
        """
        混合策略分割实现（适用于测试数据，无标签）
        参数:
            lines: CoNLL格式行列表（每行格式：词\t标签）
            max_length: 最大字符长度（硬限制）
            min_length: 最小语义单元长度（建议值）
        ...
        for line in lines:
            if line.strip():
                word= line.strip()  # 直接取每行的词
                word_list.append(word)

        pointer = 0
        while pointer < len(word_list):
            word = word_list[pointer]
            word_len = len(word)

            # 情况1：遇到超长词（强制单独成段）
            if word_len > max_length:
                if current_words:  # 先处理已有缓存
                    texts.append("".join(current_words))
                    current_words = []
                    current_len = 0
                texts.append(word)
                pointer += 1
    ...
    def tokenizer(self, tokenizer, texts):
        ...
        for item in texts:
            inputs = tokenizer(item, return_tensors="pt")
            texts_id.append(inputs["input_ids"][:, 1:-1])
            token_type_ids.append(inputs["token_type_ids"])
            attention_mask.append(inputs["attention_mask"][:, 1:-1])

        return texts_id, token_type_ids, attention_mask

    #测试数据预处理总步骤：
    def process_test_data(self,file_path,tokenizer):
        # 读取数据
        lines = self.read_conll_file(file_path)
        # 获取固定长度的文本与标签：
        texts= self.split_into_segments2_testdata(lines)
        # 获取文本tokenizer之后的结果
        texts_id, token_type_ids, attention_mask = self.tokenizer(tokenizer, texts)
        max_length = self.max_length
        for i in range(len(texts)):
            # 需要填充的长度：
            padding_length = max_length - len(texts[i])
            # texts_id填充：torch【1，len(label_ids[i])]  ——> torch[1,max_length]
            new_elements = torch.tensor([[tokenizer.pad_token_id] * padding_length])
            texts_id[i] = torch.cat((texts_id[i], new_elements), dim=1)
            # attention_mask填充: torch【1，len(label_ids[i])]  ——> torch[1,max_length]
